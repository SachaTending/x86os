...
{
    void Init() {
        pci_dev_t dev = PCI::Get(0x8086, 0x2922, PCI_TYPE_SATA);
        if (dev.bits == 0) {
            log.info("AHCI Controller not found.\n");
			return;
        }
        PCI::BusMasterEnable(dev);
        g_HBA_abar = (HBA_MEM *)PCI::Read(dev, PCI_BAR5);
        probe_port(g_HBA_abar);
    }
} // namespace AHCI
